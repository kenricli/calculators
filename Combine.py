...
# -----------------------------
st.sidebar.title("Calculator Selector")
calculator_choice = st.sidebar.radio(
    "Choose a calculator",
    ["HAI Pump Calculator", "5-FU Infusion Pump Calculator"],
    index=0,
)
...
# HAI Pump Calculator
# -----------------------------
def render_hai_calculator() -> None:
    ...
    # Disagreement screen
    if st.session_state.hai_disclaimer_agreed is False:
        st.title("🩺 HAI Pump Calculator")
    ...
    if real_weight is not None and height_cm is not None and gender and dose_rate is not None:
        ...
        if is_overweight:
            st.warning(
                f"⚠️ Patient is >35% over IBW. Using **Average Body Weight**: {dosing_weight:.1f} kg (IBW: {ibw:.1f} kg)."
            )
        else:
            st.info(
                f"✅ Patient weight is within standard dosing limits. Using **Actual Body Weight**: {real_weight} kg (IBW: {ibw:.1f} kg)."
            )
        ...
        st.warning(
            "⚠️ Please enter patient weight, height, and select a gender to generate the dosage calculations and compounding summary."
        )
...
# -----------------------------
def render_5fu_calculator() -> None:
...
# -----------------------------
if calculator_choice == "HAI Pump Calculator":
    render_hai_calculator()
else:
