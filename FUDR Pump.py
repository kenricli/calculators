...
if real_weight is not None and real_weight > 0 and height_cm is not None and height_cm > 0 and gender is not None:
    ...
    if is_overweight:
        ...
        weight_status_msg = f"⚠️ Patient is >35% over IBW. Using **Average Body Weight (ABW)**: {dosing_weight:} kg (IBW: {ibw:} kg)."
    else:
        dosing_weight = real_weight
        weight_status_msg = f"✅ Patient weight is within standard dosing limits. Using **Actual Weight**: {real_weight} kg (IBW: {ibw:} kg)."
